...
from yt_fetcher.models import AppError, SubtitleRow
from yt_fetcher.utils import clean_subtitle_text, normalize_timestamp
...
def parse_subtitle_file(path: Path) -> list[SubtitleRow]:
    ...
    if suffix == ".srt":
        ...
        raise AppError(f"Unsupported subtitle format: {path.suffix}")
    if not rows:
        raise AppError(f"Subtitle parsing resulted in empty output: {path}")
    return rows
