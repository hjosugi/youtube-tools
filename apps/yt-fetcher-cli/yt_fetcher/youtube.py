...
def download_media(
    ...
    if dl_sub and not sub_path:
        raise AppError(
            f"Subtitle file not found. The video might not have English subtitles. Files found: {files_in_dir}"
        )
    if dl_mp3 and not mp3_path:
        raise AppError(
            f"MP3 audio file could not be generated. yt-dlp might have failed. Files found: {files_in_dir}"
        )
    if dl_mp4 and not mp4_path:
        raise AppError(
            f"MP4 video file could not be generated. yt-dlp might have failed. Files found: {files_in_dir}"
        )
